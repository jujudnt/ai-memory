...
3. Click **Connecter**, then choose **Google Drive**, **Dropbox**, **OneDrive**, **iCloud Drive** or a local-folder option. Google Drive, Dropbox and OneDrive open a browser authorization under the name **rclone** and create the `AI-Memory` folder. iCloud Drive online uses rclone credentials, separate from the iCloud account signed in to macOS; with Apple double authentication, validate the prompt on your Apple device, enter the 2FA code in the field that appears, then click **Confirmer le code iCloud**. The “du Mac” options use the official local sync folder already installed on the Mac. Once connected, **Reglages > Dossier de sauvegarde** shows the exact cloud folder and lets you move future backups to another folder; AI Memory copies the current conversation set to the new location and leaves the old folder untouched.
...
If **iCloud Drive du Mac** reports that a folder is temporarily unavailable (including `Resource deadlock avoided`), or displays an incomplete local listing while files are still arriving, AI Memory requests the missing item from macOS and retries. If it remains unavailable, synchronization waits for the next watcher attempt without marking the backup complete or cleaning local copies. In Finder, open **iCloud Drive > AI-Memory** and select **Download Now** if the wait persists. Both Macs must use the same iCloud account and backup folder.
...
**iCloud Drive online:** use the normal Apple ID password. AI Memory first asks Apple to start the login and keeps that exact authentication session pending. After Apple shows a 2FA code on a trusted device, the Apple ID and password fields disappear, the code field appears, and the button becomes **Confirmer le code iCloud**. Confirmation continues the original session with only the six-digit code; it does not start another login or request another code. With Advanced Data Protection enabled, Apple also requires **Access iCloud Data on the Web** to be enabled under Apple Account > iCloud and may show an additional approval prompt. If Apple accepts 2FA but returns a partial Web session, sign in at [icloud.com](https://www.icloud.com/), accept any updated iCloud terms, then use **J'ai accepté, relancer**. AI Memory reuses the locally encrypted credentials to create a fresh Apple session and only asks for the new 2FA code. App-specific passwords are not accepted by rclone's iCloud Drive backend. If you want the iCloud account already connected to this Mac, choose **iCloud Drive du Mac** instead. See [rclone's iCloud Drive documentation](https://rclone.org/iclouddrive/#advanced-data-protection).
